...
const App = () => {
  ...
  const handleGeneratePDFInvoice = (student: Student) => {
    ...
    const handleGeneratePastMonthsInvoice = (student: Student) => {
        ...
        const baseFee = schedule.price;
        const extendedScheduleFee = student.extendedSchedule ? 30 : 0;
        const monthlyFee = baseFee + extendedScheduleFee;
        
        const tableRows = monthsToInvoice.map(d => {
            const isFirstMonth = d.getMonth() === startDate.getMonth() && d.getFullYear() === startDate.getFullYear();
            const enrollmentFee = !student.enrollmentPaid || isFirstMonth ? 100 : 0;
            return [`Jardín de infancia (${d.toLocaleString('es-ES', { month: 'long', year: 'numeric' })})`, "1", `${(monthlyFee + enrollmentFee).toFixed(2)} ${config.currency}`, `${(monthlyFee + enrollmentFee).toFixed(2)} ${config.currency}`];
        });

        const totalAmount = tableRows.reduce((sum, row) => sum + parseFloat(row[3]), 0);
