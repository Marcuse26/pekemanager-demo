...
// Ya no necesitamos definir las props aquí
const Dashboard = () => {
    // Obtenemos los datos directamente del contexto
    const { students, attendance, invoices, schedules, config } = useAppContext(); // [!code ++]
    ...
    const distinctColors = [
        '#E6194B', '#3CB44B', '#F58231', '#4363D8', '#FFE119', '#911EB4', '#F032E6', 
        '#42D4F4', '#FABEBE', '#469990', '#DBF876', '#BCF60C', '#FFD8B1', '#000075', 
        '#A9A9A9', '#800000', '#AAFFC3', '#808000', 
    ];
    ...
    const occupancyChartOptions: ChartOptions = { 
        responsive: true, maintainAspectRatio: false, plugins: { legend: { display: true, position: 'bottom', labels: { boxWidth: 12, padding: 15, font: { size: 10 }}}} 
    };

    const paymentChartOptions: ChartOptions = { 
        responsive: true, maintainAspectRatio: false, plugins: { legend: { display: true, position: 'bottom', labels: { boxWidth: 12, padding: 20 } } } 
    };

    const webeaFooterContainer: React.CSSProperties = {
        padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', marginTop: '30px'
    };
    ...
    return (
        <div>
            ...
            <div style={webeaFooterContainer}>
                ...
                <p style={webeaText}><span style={{fontWeight: '500'}}>Soporte:</span> webea.oficial@gmail.com</p>
            </div>
        </div>
    );
};
